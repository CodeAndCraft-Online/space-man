...
void setup() {
    ...
    Serial.println("Module Status Check:");
    Serial.print("- HC-06 VCC connected to Arduino 5V: ");
    Serial.println("Check wiring (red wire)");
    Serial.print("- HC-06 GND connected to Arduino GND: ");
    Serial.println("Check wiring (black wire)");
    Serial.print("- HC-06 TX connected to Arduino Pin 10: ");
    Serial.println("Check wiring (blue wire)");
    Serial.print("- HC-06 RX connected to Arduino Pin 11: ");
    Serial.println("Check wiring (green wire)");
    Serial.print("- HC-06 KEY/EN connected to Arduino 5V: ");
    Serial.println("MUST be connected for AT mode (yellow wire)");
    ...
    // Check current name setting
    btSerial.println("AT+NAME?");
    delay(200);
    if (btSerial.available()) {
        String response = btSerial.readStringUntil('\n');
        Serial.println("Current HC-06 name: " + response);
    } else {
        Serial.println("Could not check current name - key/EN pin connection needed");
    }
    ...
    Serial.println("Current PIN: 1234 (set in sketch)");
}
